...
class ExchangeServer():
    ...
    def server(self, ex_ip, ex_port, host_list_dic):
        ...
        while True:
            ...
            ExchangeServer().session_created(c, self.addr, host_list_dic)
        s.close()

    def session_created(self, soecket, addr, host_list_dic):
        threadName = 'Exchange-Client '+str(addr[0])+':'+ str(addr[1])
        try:
            t = threading.Thread(target=HelloExchangeClient,
                                name=threadName,
                                args=(soecket, addr, host_list_dic))
            t.start()
        except:
